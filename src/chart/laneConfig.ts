...
export function generateLaneConfig(keyMode: KeyMode): LaneConfig[] {
  ...
  > = {
    ...
    '24K': {
      ...
      colors: Object.fromEntries(
        Array.from({ length: 24 }, (_, i) => [(i + 1).toString(), i % 2 === 0 ? '#ffffff' : '#333333'])
      ),
      laneWidth: 10,
      scratchWidth: 0,
    },
    '48K': {
      ...
      colors: Object.fromEntries(
        Array.from({ length: 48 }, (_, i) => [(i + 1).toString(), i % 2 === 0 ? '#ffffff' : '#333333'])
      ),
      laneWidth: 5,
      scratchWidth: 0,
    },
  ...
  return config.lanes.map((id, index) => {
...
export function getLaneIds(keyMode: KeyMode): string[] {
  return generateLaneConfig(keyMode).map((l) => l.id);
}
